-- ============================================
-- SCHEMA COMPLETO — Gestionale B&B
-- ============================================
...
CREATE TABLE camere (
  ...
  stato             VARCHAR(20)  NOT NULL DEFAULT 'Pulita'
                    CHECK (stato IN ('Pulita','Da pulire','In manutenzione'))
);
...
CREATE TABLE ospiti (
  ...
  tipo_documento     VARCHAR(20)  NOT NULL
                     CHECK (tipo_documento IN ('Carta d''Identità','Passaporto','Patente')),
...
CREATE TABLE prenotazioni (
  ...
  stato             VARCHAR(30)  NOT NULL DEFAULT 'Confermata'
                    CHECK (stato IN ('Confermata','Check-in effettuato','Check-out effettuato','Annullata','No-show')),
  canale            VARCHAR(20)  NOT NULL DEFAULT 'Diretto'
                    CHECK (canale IN ('Diretto','Booking','Airbnb','Expedia','Altro')),
  note_prenotazione TEXT,
  tassa_soggiorno   DECIMAL(6,2) NOT NULL DEFAULT 0,
  ospiti_esenti     SMALLINT     NOT NULL DEFAULT 0,
  id_sconto         INT REFERENCES sconti(id_sconto),
  sconto_applicato  DECIMAL(6,2) NOT NULL DEFAULT 0
);

-- 7. Dettagli Prenotazione
...
CREATE TABLE cassa (
  ...
  metodo_pagamento   VARCHAR(20)  NOT NULL
                     CHECK (metodo_pagamento IN ('Contanti','Carta','Bonifico','PayPal'))
);
